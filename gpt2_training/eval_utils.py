...
def eval_model_loss(model, tokenizer, eval_dataloader, epoch_id, args):
    ...
    with torch.no_grad():
        for step, batch in enumerate(eval_dataloader):
            ...
            loss, ppl,lm_logits,lm_labels = model(input_ids, position_ids, token_ids, label_ids)
            logger.info('lm_logits: '+str(lm_logits.shape))
            logger.info('lm_labels: ' + str(lm_labels.shape))
